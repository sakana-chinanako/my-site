    ...
    for (const e of entries) {
      ...
      if (e.isDirectory()) {
        ...
        if (isDoc || isImg) {
          ...
          const sidecarPath = path.join(path.dirname(fp), path.basename(e.name, ext) + '.json');
          if (fs.existsSync(sidecarPath)) {
            try {
              const sidecar = JSON.parse(fs.readFileSync(sidecarPath, 'utf-8'));
              if (Array.isArray(sidecar.tags)) tags = sidecar.tags;
              if (typeof sidecar.description === 'string') description = sidecar.description;
              if (isImg && !meta) meta = sidecar;
            } catch (e2) {
              console.warn('⚠️ 侧车 JSON 失败:', sidecarPath, e2.message);
            }
          }
          items.push({
            name: e.name,
            category: cat,
            path: "/" + rp.split('/').map(s => encodeURIComponent(s)).join('/'),
            type: type,
            ext: ext.replace(".", ""),
...
function cleanupEmptyDirs(dir) {
  try {
    ...
    for (const e of entries) {
      ...
      if (e.isDirectory()) {
        if (cleanupEmptyDirs(fp)) hasContent = true;
      } else {
        const ext = path.extname(e.name).toLowerCase();
        if (ALLOWED_DOC_EXT.includes(ext) || ALLOWED_IMG_EXT.includes(ext)) {
          hasContent = true;
        }
      }
    }
    if (!hasContent && dir !== path.join(publicDir, 'docs') && dir !== path.join(publicDir, 'images')) {
      fs.rmdirSync(dir);
      console.log('🗑️ 清理空目录:', path.relative(publicDir, dir));
      return false;
    }
    return hasContent;
  } catch (e) {
    console.warn('⚠️ 清理空目录失败:', path.relative(publicDir, dir), e.message);
    return true;
  }
}

console.log('🔍 清理空目录...');
cleanupEmptyDirs(path.join(publicDir, 'docs'));
cleanupEmptyDirs(path.join(publicDir, 'images'));
...
for (const f of oldIndex.files) {
  if (f.fullText) oldTextMap.set(decodeURIComponent(f.path), f.fullText);
}
